...
public interface CourseRepository
    ...
    @Query("SELECT c FROM Course c JOIN c.modules m WHERE c.topicId = :topicId")
    ...
    @Query(value = "SELECT COUNT(l) FROM lesson l " +
            ...
            "WHERE c.topic_id = :topicId", nativeQuery = true)
    long countLessonsByTopicId(Long topicId);

    @Query(value = "SELECT COUNT(m) FROM module m " +
            "JOIN course c ON m.course_id = c.id " +
            "WHERE c.topic_id = :topicId", nativeQuery = true)
    long countModulesByTopicId(Long topicId);

    @Query("SELECT c FROM Course c WHERE c.topicId = :topicId")
